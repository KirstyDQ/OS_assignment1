...
void *producer(void *a)
{
  ...
  sem_wait(empty1);
  sem_wait(mutex1);
  ptr1->buff[ptr1->in] = in_data;
  printf("PID is %d | TID is %lu | in_data is %d\n", getpid(), (unsigned long)pthread_self(), ptr1->buff[ptr1->in]);
...
/*  sem_unlink("full1");
  ...
  ptr1 = (product*)mmap(0, sizeof(product), PROT_WRITE, MAP_SHARED, shm_fd, 0);
  if (ptr1 == MAP_FAILED){
    perror("mmap error!!!\n");
    exit(1);
    }
  ...
  for(loop = 0; loop<10;loop++){
    for(i=0; i<ThreadNum; i++){pthread_create(&tid[i], &attr, producer, argv[1]);}
